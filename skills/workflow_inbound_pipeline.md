...
飞书需求从入站到**提需人验收**的自动化闭环（A→F + Acceptance）：

```
A 入站 → B 分诊 → C 分析 → D 执行+PR → E Gate Review（AI）
    → 人合 dev → dev CI/CD → F sole assign 负责人 + 验收 comment
    → Acceptance（accept pass|fail）→ dev-accepted + promote PR → prod merge → F prod 通知
```

给人读的详细说明见本文件各 Pipeline 节；E/F/Acceptance 亦可单独查阅：

- [Gate Review（E）](./workflow_gate_review.md)
- [Human Gate（Legacy）](./workflow_human_gate.md)
- [Dev Acceptance](./workflow_acceptance.md)
...
## Pipeline Acceptance（F 之后，新路径）

F dev handback 后，提需人（或负责人代操作）跑 `accept`：

```bash
source scripts/load_asp_env.sh
./venv/bin/feishu-inbound accept pass --config tools/feishu_inbound/config.yaml --issue <N> --repo <repo>
```

详见 [Dev Acceptance](./workflow_acceptance.md)。Legacy 人测 gate 见 [Human Gate](./workflow_human_gate.md)。
