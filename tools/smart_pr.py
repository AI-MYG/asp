...
def main() -> None:
    ...
    # Always link with "References" (never "Closes") so merging the PR never
    # auto-closes the issue. The Pipeline D workflow hands the issue back to its
    # requester for verification; closing is a human decision after acceptance.
    pr_body = f"References {args.issue_repo}#{args.issue}{model_line}\n\n🤖 Generated by Pipeline D (Smart PR)"
    pr_cmd = [
        "gh", "pr", "create",
        "-R", repo,
        "--base", base_branch,
