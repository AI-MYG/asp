...
- **执行**: 经 rootgrove AgentClient **委托到与 Pipeline D 不同的 Agent 平台**（如 D=Cursor、E=Claude Code；OpenCode 为 E fallback）读 PR diff + analysis 做 gate review；**只 review，不改代码/不 merge**
- **状态机**:
