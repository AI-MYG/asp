...
```
            ...
            Pipeline E Gate Review (review model ≠ executor)
              ...
              └─ 打回 → 去 executed + Gate Review comment → 回 D 修订
                                 │
                                 ↓
            Pipeline F（PR merged + dev CI/CD success）
              → sole assignee 提需人 + 验收 comment
```
...
- **触发**: `review-dev-pass`；PR 已 merge；dev CI/CD 在 merge commit 上 success；由 **Lead tick**（`:20/:50`）链式调用
- **执行**: 读 GitHub Actions 结论 + `handback_to_requester`（sole assignee → 创建者 + 验收 comment）
- **幂等**: sole assignee 已是 author 则跳过（**不再**使用 `ready-for-acceptance` label）
- **不做**: merge、改代码
- **脚本**: `tools/feishu_inbound/issue_dev_handback.py`；配置 `config.yaml` → `pipeline_f`
